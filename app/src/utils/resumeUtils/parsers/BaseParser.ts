...
export abstract class BaseParser {
  ...
  protected splitIntoSections(): Map<string, string> {
    ...
    this.text.replace(sectionRegex, (match, sectionTitle, index) => {
      if (lastSectionTitle) {
        sections.set(
          lastSectionTitle.trim().toLowerCase(),
          this.text.slice(lastIndex, index).trim()
        );
      }
    ...
    if (lastSectionTitle) {
      sections.set(
        lastSectionTitle.trim().toLowerCase(),
        this.text.slice(lastIndex).trim()
      );
    }
  ...
  protected parseSection(sectionContent: string): ResumeSection[] {
    ...
    return entries.map((entry) => {
      ...
      const dateMatch = title.match(
        /\b(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec|January|February|March|April|May|June|July|August|September|October|November|December)\.?\s+\d{4}\s*(-|–|to)?\s*(Present|\w+\.?\s+\d{4})?\b|\b\d{1,2}\/\d{4}\s*(-|–|to)?\s*(\d{1,2}\/\d{4}|Present)?\b/i
      );
      const date = dateMatch ? this.formatDate(dateMatch[0]) : "";
      const companyOrInstitution = title
